...
<Callout>
Les **plugins** sont des fonctions qui effectuent une seule et unique transformation sur un morceau de code. Un pipeline Babel typique consiste en plusieurs plugins qui transforment le code source d'entrée de manière séquentielle.
</Callout>

<Callout>
Les **presets** sont des groupements de plugins rassemblés pour être plus pratiques à utiliser pour certains cas d'usages spécifiques. Par exemple, `@babel/preset-env` transpile le JavaScript moderne vers une version compatible avec les navigateurs ciblés, `@babel/preset-react` gère le JSX et `@babel/preset-typescript` transpile le TypeScript en JavaScript. Chacun de ces presets n'est qu'une liste pré-définie de plugins individuels appliqués en séquence sur le code d'entrée.
</Callout>
...
<Callout>
Avec Webpack, **la plupart des opérations de transpilation sont effectuées au moyen de loaders**. Webpack lui-même est responsable de construire le graphe des dépendances, mais chaque fichier passe à travers un ou plusieurs loaders afin d'être transformé avant d'être bundlé.
</Callout>
...
<ImagePair>
  <Image src={buildPerformance} alt="Comparatif de performance de build en production" />
  <Image src={devBuildPerformance} alt="Comparatif de performance de build en développement" />
</ImagePair>
