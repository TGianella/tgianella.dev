...
registerScrollStrategy({
  ...
  save: () => {
    ...
    const index = children.findIndex(el => el.getBoundingClientRect().bottom > navHeight);
    if (index === -1) return null;

    return () => {
      const cs = [...(document.querySelector('.prose')?.children ?? [])] as HTMLElement[];
      const el = cs[Math.min(index, cs.length - 1)];
      if (!el) return;
      const nav = document.querySelector<HTMLElement>('.nav');
      window.scrollTo({ top: el.offsetTop - (nav?.offsetHeight ?? 0), behavior: 'instant' });
    };
  },
});
